...
% BP is the number of input/output ports
\pgfkeys{/tikz/.cd,%
      BP/.store in=\BP,%
      BP=8,%
}%

% BM is the number of ports per middle layer switch
\pgfkeys{/tikz/.cd,%
      BM/.store in=\BM,%
      BM=2,%
}%

% BN is the number of ports per all other switchers
\pgfkeys{/tikz/.cd,%
      BN/.store in=\BN,%
      BN=4,%
}%

% BL is the number of layers per wing ((total-1)/2)
\pgfkeys{/tikz/.cd,%
      BL/.store in=\BL,%
      BL=1,%
}%
...
\tikzset{butterfly/.code={        

      % Number of modules per stage
      \pgfmathtruncatemacro{\mswitches}{\BP/\BM}
      \pgfmathtruncatemacro{\aswitches}{\BP/\BN}
      
      % Shrinkage of middle stage nodes
      \pgfmathtruncatemacro{\ns}{\BN/\BM}
      
      % Number of stages
      \pgfmathtruncatemacro{\stages}{2*\BL+1}
      
      \pgfmathtruncatemacro{\midstage}{\BL+1}
      
      % MODULES for all stages
      \foreach \s [evaluate=\s as \numstage using int(\s-1)] in {1,...,\stages}{
          \ifnum\s=1
          % FIRST MODULE 
           \foreach \i in {1,...,\aswitches}{
                 \path let \n1 = {int(0-\i)}, \n2={0-\i*\moduleysep*\BN/4}
                 in
                 node[module,#1,module opacity,yshift=1cm,minimum height=0.25cm*\BN]  (r\s-\i) at +(0,\n2) {\pgfmathparse{int(multiply(\n1,-1))}\pgfmathresult};
                 
                 % INPUTS MODULE 1    
               % the number of inputs module one is exactly \mone
               \pgfmathsetmacro\roneintervalspace{1/(\BN+1)}
               \foreach \roneinput[evaluate=\roneinput as \roneinterval using \roneintervalspace*\roneinput] 
               in {1,...,\BN}
               \draw ($(r1-\i.north west)!\roneinterval!(r1-\i.south west)-(0.5*\pinlength,0)$)node[scale=0.1](r1-\i-front input-\roneinput){}--($(r1-\i.north west)!\roneinterval!(r1-\i.south west)$) node[circle,draw,scale=0.1] (r1-\i-input-\roneinput) {};
          ...
          \ifnum\s=\stages
           ...
           \foreach \i in {1,...,\aswitches}{
                 \path let \n1 = {int(0-\i)}, \n2={0-\i*\moduleysep*\BN/4}
                 in
                 node[module,#1,module opacity,yshift=1cm,minimum height=0.25cm*\BN]  (r\s-\i) at +(\numstage*0.6*\modulexsep,\n2) {\pgfmathparse{int(multiply(\n1,-1))}\pgfmathresult};
                 
                 % INPUTS MODULE \s
               % the number of inputs of module three is the number of modules stage 2 \rtwo
               \pgfmathsetmacro\rintervalspace{1/(\BN+1)}
               \foreach \rinput[evaluate=\rinput as \rinterval using \rintervalspace*\rinput] 
               in {1,...,\BN}
               \node[circle,draw,scale=0.1] (r\s-\i-input-\rinput)at($(r\s-\i.north west)!\rinterval!(r\s-\i.south west)$)  {};
        ...
        \ifnum\s=\midstage
           % MIDDLE MODULEs 
           \pgfmathsetmacro\midysep{(\moduleysep*\BM/4))}
           \foreach \i in {1,...,\mswitches}{
                \pgfmathparse{mod(\i,\ns)!=0}
                \let\cond\pgfmathresult
                \pgfmathparse{\BM==\BN}
                \let\condB\pgfmathresult
                 \path let \n1 = {int(0-\i)}, \n2={0-(\i*\moduleysep*\BM/4)-((\moduleysep*(\BN/12))*\cond)-(((\moduleysep*(\BN/12))/2)*(1-\cond))+((\moduleysep*(\BN/24))*\condB)}%-\cond*(\i/\ns)*\moduleysep*\BN/4+\cond*\moduleysep*\BN/4}
                 in
                 node[module,module size=1cm/\ns,module opacity,yshift=1cm,minimum height=0.25cm*\BM]  (r\s-\i) at +(\numstage*0.6*\modulexsep,\n2) {\pgfmathparse{int(multiply(\n1,-1))}\pgfmathresult};
                 
                 % INPUTS MODULE \s
               % the number of inputs of module three is the number of modules stage 2 \rtwo
               \pgfmathsetmacro\rintervalspace{1/(\BM+1)}
               \foreach \rinput[evaluate=\rinput as \rinterval using \rintervalspace*\rinput] 
               in {1,...,\BM}
               \node[circle,draw,scale=0.1] (r\s-\i-input-\rinput)at($(r\s-\i.north west)!\rinterval!(r\s-\i.south west)$)  {};
        ...
        \fi
        %\pgfmathparse{and(and(\s>1,\s<\stages),\s!=\BL+1)}
        \pgfmathparse{and(and(\s>1,\s<\stages),\s!=\BL+1)}
        \let\cond\pgfmathresult
        \ifnum\cond=1
           % INTERMEDIATE MODULEs 
